...
class ExpResults(Page):
    ...
    def vars_for_template(player: Player):
        if player.round_number == 1:
            ...
            return dict(selectedVal=player.sellingPrice, payoff=amount, list=C.TABLE_ONE)
        elif player.round_number == 2:
            amount = cal_amount2(player)
            player.payoff = amount
            return dict(selectedVal=player.sellingPrice, payoff=amount, list=C.TABLE_TWO)
        else:
            amount = cal_amount3(player)
            player.payoff = amount
            return dict(selectedVal=player.sellingPrice, payoff=amount, list=C.TABLE_THREE)
