    ...
    def live_method(player: Player,data):
        if data["first"]==1:
            ...
            if data["select_type"]=="A":
                player.risk_List=risklist[:int(data["position_num"])-1 ] + 'A' + risklist[int(data["position_num"]):]
            else:
                player.risk_List=risklist[:int(data["position_num"])-1 ] + 'B' + risklist[int(data["position_num"]):]
            player.num_A=player.risk_List.count("A")
...
class Decision_3(Page):
    form_model = 'player'
    form_fields = ['u_individual_choice',
                   'individual_choice_u_comment'
                  ]

class Decision_4(Page):
    form_model = 'player'
    form_fields = ['s_individual_choice',
                   'individual_choice_s_comment'
                  ]

class Decision_5(Page):
    form_model = 'player'
    form_fields = ['e_individual_choice',
                   'individual_choice_e_comment'
                  ]
...
class Results(Page):
    ...
    def js_vars(player: Player):
        ...
        print(num_participants, r_count_A0, r_count_A1, r_count_A2, r_count_A3, r_count_A4, r_count_A5)
        ...
        u_prop_num_A, u_prop_num_B = keiosan_ratio(u_count_A, u_count_B, num_participants_u)
        print("Decision_3:不確実性の質問グラフー",u_prop_num_A, u_prop_num_B)

        # Decision_4:10倍の質問グラフー－－－－－－－
        ...
        s_prop_num_A, s_prop_num_B = keiosan_ratio(s_count_A, s_count_B, num_participants_s)
