...
class Group(BaseGroup):
    ...
    p2_decision_why  = models.LongStringField(
        label="なぜその選択をしましたか？"
    )
...
class Player(BasePlayer):
    ...
    tmp_first_player_why = models.LongStringField(
        label="なぜその選択をしましたか？"
    )

    tmp_second_player = models.StringField(
        choices=C.CHOICE_LIST_GOTE_TMP,
        widget=widgets.RadioSelect,
        label=""
    )
    tmp_second_player_why = models.LongStringField(
        label="なぜその選択をしましたか？"
    )
...
class Strategy(Page):
    ...
    form_model = 'player'
    form_fields = ['tmp_first_player', 'tmp_first_player_why', 'tmp_second_player', 'tmp_second_player_why']
