...
# FUNCTION------------------------------
def da_algorithm(group, student_choices, totyu_studentStatus, totyu_teacherStatus,student_choices1):
                ...
                if len(totyu_teacherStatus[t]) < C.capacity_lim[t]:  # 定員に空きがあれば・・・
                    ...
                    for x in totyu_teacherStatus[t]:  # 現在マッチしている学生の中で最も順位が低い学生をcurrentPartnerに
                        if totyu_teacherStatus[t].index(x) == 0:
                            currentPartner = x
                        elif C.teacher_choices[t].index(x) >= C.teacher_choices[t].index(currentPartner):
                            currentPartner = x
        ...
        if len([1 for y in totyu_studentStatus if totyu_studentStatus[y] is None]) != 0:
            da_algorithm(group, student_choices, totyu_studentStatus, totyu_teacherStatus,student_choices1)
        else:
            # DAの結果を格納
