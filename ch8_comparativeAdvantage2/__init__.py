...
class Constants(BaseConstants):
    name_in_url = 'ch8_comparativeAdvantage2'
        ...
        if closedTransaction == False:
            ...
            for p in players:
                ...
                if p.id_in_group == yourID:
                    p.cheeze = round(p.cheeze + sell, 2)  # 相手の現金に買った分を足す
                    p.bread -= 1  # 資産が1つ減る
                    if Constants.autoClearFlg == 1:
                        ...
                        for num in range(p.sellCount + 1):
                            ...
                            if flg:
                                ...
                                flg = True
            torihikigaku = sell
            player.cheeze = round(player.cheeze - sell, 2)  # 自分の現金から買った分を引く
        ...
        if closedTransaction == False:
            ...
            for p in players:
                if p.player_type != data['player_type']:
                    continue
                if p.id_in_group == yourID:
                    p.cheeze = round(p.cheeze - buyValue, 2)  # 相手の現金に買った分を足す
                    p.bread += 1  # 資産が1つ減る
                    if Constants.autoClearFlg == 1:
                        p.sellCount = 0
                        p.buyCount = 0
            ...
            player.cheeze = round(player.cheeze + buyValue, 2)  # 自分の現金から買った分を引く
            player.bread -= 1  # 資産が1つ増える
        response = dict(
            formSyubetu='sell',
            formData=value,
    ...
    def error_message(player: Player, values):
        ...
        if player.player_type == 'A':
            if (
                ...
                + values['cheeze'] * Constants.cheeze_cost_A)
            ...
            if (
                ...
                + values['cheeze'] * Constants.cheeze_cost_B)
                != Constants.working_hours
            ):
                return '20時間丁度になるように入力してください'
